...
Class Pedido {
	...
	public function listar(){
		$sql="SELECT 'worpress' AS tipo_pedido,p.ID AS pedidoID,p.post_date_gmt AS fecha,p.post_status AS estado,CONCAT(pm_first.meta_value, ' ', pm_last.meta_value) AS cliente,pm_total.meta_value AS total,fac.id AS facturaID,fac.status AS status_factura,fac.nombrePDF,fac.nombreXML,fac.folioFiscal FROM wp_posts p LEFT JOIN wp_postmeta pm_first ON p.ID = pm_first.post_id AND pm_first.meta_key = '_billing_first_name' LEFT JOIN wp_postmeta pm_last ON p.ID = pm_last.post_id AND pm_last.meta_key = '_billing_last_name' LEFT JOIN wp_postmeta pm_total ON p.ID = pm_total.post_id AND pm_total.meta_key = '_order_total' LEFT JOIN factura fac ON p.ID = fac.pedido_id WHERE p.post_type = 'shop_order' AND p.post_status!='wc-pending' AND p.post_status!='wc-cancelled' AND p.post_date >= '2025-01-01' GROUP BY p.ID
		UNION ALL
