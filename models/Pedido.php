...
Class Pedido {
	...
	public function editar_factura($facturaID,$serie,$metodoPago,$claveTipoComprobante,$usoCfdi,$formadePago,$descuento,$moneda,$tipoCambio,$cliente_id,$razonSocial,$rfcCliente,$usuarioID,$comentarios,$pedido_id,$user_login,$user_nicename,$email_cliente,$contrasenna,$telefono,$calle,$num_ext,$num_int,$colonia,$poblacion,$edoPais,$cp,$regimenFiscal,$num_cuenta,$banco,$producto_id,$descripcion,$cantidad,$precioVenta,$subtotal){
		...
		$sql_factura="UPDATE factura SET serie='$serie',metodoPago='$metodoPago',claveTipoComprobante='$claveTipoComprobante',usoCfdi='$usoCfdi',formadePago='$formadePago',descuento='$descuento',moneda='$moneda',tipoCambio='$tipoCambio',cliente_id='$cliente_id',comentarios='$comentarios',pedido_id='$pedido_id',updated_at=NOW() WHERE id='$facturaID'";
	...
	public function listar(){
		$sql="SELECT 'worpress' AS tipo_pedido,p.ID AS pedidoID,user.ID AS clienteID,p.post_date_gmt AS fecha,p.post_status AS estado,CONCAT(pm_first.meta_value, ' ', pm_last.meta_value) AS cliente,pm_total.meta_value AS total,fac.id AS facturaID,fac.metodoPago,fac.status AS status_factura,fac.nombrePDF,fac.nombreXML,fac.folioFiscal FROM wp_posts p LEFT JOIN wp_postmeta pm_first ON p.ID = pm_first.post_id AND pm_first.meta_key = '_billing_first_name' LEFT JOIN wp_postmeta pm_last ON p.ID = pm_last.post_id AND pm_last.meta_key = '_billing_last_name' LEFT JOIN wp_postmeta pm_total ON p.ID = pm_total.post_id AND pm_total.meta_key = '_order_total' LEFT JOIN factura fac ON p.ID = fac.pedido_id LEFT JOIN wp_postmeta customer ON p.ID=customer.post_id AND customer.meta_key='_customer_user' LEFT JOIN wp_users user ON user.ID=customer.meta_value WHERE p.post_type = 'shop_order' AND p.post_status!='wc-pending' AND p.post_status!='wc-cancelled' AND p.post_date >= '2025-01-01' GROUP BY p.ID
		UNION ALL
		SELECT 'manual' AS tipo_pedido,ped.id AS pedidoID,ped.cliente_id AS clienteID,ped.created_at AS fecha,ped.status AS estado,IFNULL(dat.razonSocial,ped.nombreCliente) AS cliente,det.totales AS total,fac.id AS facturaID,fac.metodoPago,fac.status AS status_factura,fac.nombrePDF,fac.nombreXML,fac.folioFiscal FROM pedidos ped INNER JOIN (SELECT pedido_id,SUM(cantidad*precioVenta) AS totales FROM pedidosDetalles GROUP BY pedido_id) det ON ped.id=det.pedido_id LEFT JOIN datos_fiscales dat ON ped.cliente_id = dat.cliente_id LEFT JOIN factura fac ON ped.id = fac.pedido_id";
