...
Class Escritorio {
	...
	public function mostrar($clienteID){
		...
		LEFT JOIN (SELECT a.cliente_id,a.id AS alumnoID,a.activo,usr.nombre AS nombreVendedor,IFNULL(a.updated_at,a.created_at) AS fechaCreacion,a.user_id,cli.nombre AS nombreCliente,cla.nombre AS nombreClase,SUM(asist.asistenciasTotales) AS asistenciasTotales,SUM(alu.asistenciasAlumnos) AS asistenciasAlumnos FROM alumnos a LEFT JOIN users usr ON a.user_id=usr.id INNER JOIN clientes cli ON cli.id=a.cliente_id LEFT JOIN (SELECT nombre,id FROM clases WHERE activo='1') cla ON a.clase_id=cla.id LEFT JOIN (SELECT ast.clase_id,COUNT(DISTINCT ast.id) AS asistenciasTotales FROM asistencias ast INNER JOIN clases cl ON ast.clase_id=cl.id LEFT JOIN asistenciaAlumno a ON ast.id=a.asistencia_id WHERE MONTH(ast.asistencia_inicio) = MONTH(CURRENT_DATE()) AND YEAR(ast.asistencia_inicio) = YEAR(CURRENT_DATE()) GROUP BY ast.clase_id) asist ON cla.id=asist.clase_id LEFT JOIN (SELECT a.alumno_id,ast.clase_id,COUNT(DISTINCT ast.id) AS asistenciasTotales,COUNT(a.alumno_id) AS asistenciasAlumnos,COUNT(DISTINCT a.alumno_id) AS alumnos FROM asistencias ast INNER JOIN clases cl ON ast.clase_id=cl.id LEFT JOIN asistenciaAlumno a ON ast.id=a.asistencia_id WHERE MONTH(ast.asistencia_inicio) = MONTH(CURRENT_DATE()) AND YEAR(ast.asistencia_inicio) = YEAR(CURRENT_DATE()) GROUP BY a.alumno_id) alu ON a.id=alu.alumno_id INNER JOIN alumnos alm ON alu.alumno_id=alm.id WHERE a.cliente_id='$clienteID' AND alm.activo='1' GROUP BY a.cliente_id) alunum ON cli.id=alunum.cliente_id WHERE cli.id='$clienteID' GROUP BY cli.id";
		return ejecutarConsultaSimpleFila($sql);
	}

	public function mostrar_todos(){
		$sql="SELECT 0 AS clienteID,'Todos' AS nombreCliente,SUM(a.numeroClases) AS numeroClases,SUM(a.numeroAlumnos) AS numeroAlumnos,SUM(a.asistenciasAlumnos) AS asistenciasAlumnos,SUM(a.asistenciasTotales) AS asistenciasTotales,correctas,preguntas FROM (
			SELECT cli.id AS clienteID,cli.nombre AS nombreCliente,IFNULL(cla.numeroClases,0) AS numeroClases,IFNULL(alu.numeroAlumnos,0) AS numeroAlumnos,IFNULL(alunum.asistenciasAlumnos,0) AS asistenciasAlumnos,IFNULL(alunum.asistenciasTotales,0) AS asistenciasTotales,IFNULL(exa.correctas,0) AS correctas,IFNULL(exa.preguntas,0) AS preguntas FROM clientes cli LEFT JOIN (SELECT exa.cliente_id,SUM(CASE WHEN resp.respuesta=resp.correcta THEN 1 ELSE 0 END) AS correctas,COUNT(resp.pregunta_id) AS preguntas FROM examenAlumno exa LEFT JOIN examenAlumnoRespuesta resp ON exa.id=resp.examen_id WHERE exa.curso_id!='0' AND resp.alumno_id!='0' GROUP BY exa.cliente_id) exa ON cli.id=exa.cliente_id LEFT JOIN (SELECT id,cliente_id,COUNT(id) AS numeroClases FROM clases WHERE activo='1' GROUP BY cliente_id) cla ON cli.id=cla.cliente_id LEFT JOIN (SELECT cliente_id,COUNT(id) AS numeroAlumnos FROM alumnos WHERE activo='1' GROUP BY cliente_id) alu ON cli.id=alu.cliente_id LEFT JOIN (SELECT a.cliente_id,a.id AS alumnoID,a.activo,usr.nombre AS nombreVendedor,IFNULL(a.updated_at,a.created_at) AS fechaCreacion,a.user_id,cli.nombre AS nombreCliente,cla.nombre AS nombreClase,SUM(asist.asistenciasTotales) AS asistenciasTotales,SUM(alu.asistenciasAlumnos) AS asistenciasAlumnos FROM alumnos a LEFT JOIN users usr ON a.user_id=usr.id INNER JOIN clientes cli ON cli.id=a.cliente_id LEFT JOIN (SELECT nombre,id FROM clases WHERE activo='1') cla ON a.clase_id=cla.id LEFT JOIN (SELECT ast.clase_id,COUNT(DISTINCT ast.id) AS asistenciasTotales FROM asistencias ast INNER JOIN clases cl ON ast.clase_id=cl.id LEFT JOIN asistenciaAlumno a ON ast.id=a.asistencia_id WHERE MONTH(ast.asistencia_inicio) = MONTH(CURRENT_DATE()) AND YEAR(ast.asistencia_inicio) = YEAR(CURRENT_DATE()) GROUP BY ast.clase_id) asist ON cla.id=asist.clase_id LEFT JOIN (SELECT a.alumno_id,ast.clase_id,COUNT(DISTINCT ast.id) AS asistenciasTotales,COUNT(a.alumno_id) AS asistenciasAlumnos,COUNT(DISTINCT a.alumno_id) AS alumnos FROM asistencias ast INNER JOIN clases cl ON ast.clase_id=cl.id LEFT JOIN asistenciaAlumno a ON ast.id=a.asistencia_id AND MONTH(ast.asistencia_inicio) = MONTH(CURRENT_DATE()) AND YEAR(ast.asistencia_inicio) = YEAR(CURRENT_DATE()) GROUP BY a.alumno_id) alu ON a.id=alu.alumno_id INNER JOIN alumnos alm ON alu.alumno_id=alm.id WHERE alm.activo='1' GROUP BY a.cliente_id) alunum ON cli.id=alunum.cliente_id
		) a";
		return ejecutarConsultaSimpleFila($sql);
	}

	public function progresoClases($clienteID){
		$sql="SELECT cla.id AS claseID,cla.cliente_id,cla.nombre AS nombreClase,cur.nombre AS nombreCurso,det.paginas_totales,IFNULL(asi.paginaCurso,0) AS paginaCurso,alu.totalAlumnos,alu.nombreAlumnos,mae.nombre AS nombreMaestro,asist.asistenciasTotales,asial.asistenciasAlumnos FROM clases cla INNER JOIN cursos cur ON cla.curso_id=cur.id INNER JOIN (SELECT curso_id,SUM(paginas) AS paginas_totales FROM cursoSecciones GROUP BY curso_id) det ON cur.id=det.curso_id LEFT JOIN (SELECT a.clase_id,cur.nombre AS nombreCurso,MAX(a.numero_pagina) AS paginaCurso,cl.curso_id FROM asistencias a INNER JOIN clases cl ON a.clase_id=cl.id INNER JOIN cursos cur ON cl.curso_id=cur.id WHERE cl.curso_id=a.curso_id GROUP BY a.clase_id) asi ON cla.id=asi.clase_id
		
		LEFT JOIN (SELECT COUNT(id) AS totalAlumnos,GROUP_CONCAT(DISTINCT nombre SEPARATOR ', ') AS nombreAlumnos,clase_id FROM alumnos WHERE activo='1' GROUP BY clase_id) alu ON cla.id=alu.clase_id INNER JOIN users mae ON cla.maestro_id=mae.id

		LEFT JOIN (SELECT asist.clase_id,ROUND(COUNT(a.alumno_id)/COUNT(DISTINCT a.alumno_id) ,0) AS asistenciasAlumnos FROM asistenciaAlumno a INNER JOIN asistencias asist ON asist.id=a.asistencia_id INNER JOIN alumnos alm ON a.alumno_id=alm.id WHERE alm.activo='1' AND MONTH(asist.asistencia_inicio) = MONTH(CURRENT_DATE()) AND YEAR(asist.asistencia_inicio) = YEAR(CURRENT_DATE()) GROUP BY asist.clase_id) asial ON cla.id=asial.clase_id
		
		LEFT JOIN (SELECT ast.clase_id,COUNT(DISTINCT ast.id) AS asistenciasTotales FROM asistencias ast INNER JOIN asistenciaAlumno a ON ast.id=a.asistencia_id INNER JOIN alumnos alm ON a.alumno_id=alm.id WHERE alm.activo='1' AND ast.asistencia_inicio>=DATE_FORMAT(CURDATE(), '%Y-%m-01') GROUP BY ast.clase_id) asist ON cla.id=asist.clase_id

		WHERE cla.cliente_id='$clienteID' AND cla.activo='1' GROUP BY cla.id ORDER BY cla.id DESC";
		return ejecutarConsulta($sql);
	}

	public function progresoClasesTodos(){
		$sql="SELECT cla.id AS claseID,cla.cliente_id,cla.nombre AS nombreClase,cur.nombre AS nombreCurso,det.paginas_totales,IFNULL(asi.paginaCurso,0) AS paginaCurso,alu.totalAlumnos,alu.nombreAlumnos,mae.nombre AS nombreMaestro,asist.asistenciasTotales,asial.asistenciasAlumnos FROM clases cla INNER JOIN cursos cur ON cla.curso_id=cur.id INNER JOIN (SELECT curso_id,SUM(paginas) AS paginas_totales FROM cursoSecciones GROUP BY curso_id) det ON cur.id=det.curso_id LEFT JOIN (SELECT a.clase_id,cur.nombre AS nombreCurso,MAX(a.numero_pagina) AS paginaCurso,cl.curso_id FROM asistencias a INNER JOIN clases cl ON a.clase_id=cl.id INNER JOIN cursos cur ON cl.curso_id=cur.id WHERE cl.curso_id=a.curso_id GROUP BY a.clase_id) asi ON cla.id=asi.clase_id
		
		LEFT JOIN (SELECT COUNT(id) AS totalAlumnos,GROUP_CONCAT(DISTINCT nombre SEPARATOR '<br>') AS nombreAlumnos,clase_id FROM alumnos WHERE activo='1' GROUP BY clase_id) alu ON cla.id=alu.clase_id INNER JOIN users mae ON cla.maestro_id=mae.id
	...
	public function listadoAlumnos($clienteID){
		...
		LEFT JOIN (SELECT alm.id AS alumno_id,COUNT(DISTINCT ast.id) AS asistenciasTotales FROM alumnos alm INNER JOIN asistencias ast ON alm.clase_id=ast.clase_id WHERE ast.asistencia_inicio>=DATE_FORMAT(CURDATE(), '%Y-%m-01') GROUP BY alm.id) asist ON a.id=asist.alumno_id /* ast.created_at>=IFNULL(alm.updated_at,alm.created_at) asistencias globales */
		
		LEFT JOIN (SELECT a.alumno_id,ast.clase_id,COUNT(a.alumno_id) AS asistenciasAlumnos FROM asistencias ast LEFT JOIN clases cl ON ast.clase_id=cl.id LEFT JOIN asistenciaAlumno a ON ast.id=a.asistencia_id WHERE ast.asistencia_inicio>=DATE_FORMAT(CURDATE(), '%Y-%m-01') GROUP BY a.alumno_id) alu ON a.id=alu.alumno_id 
		
		WHERE a.cliente_id='$clienteID' AND a.activo='1' GROUP BY a.id";
		return ejecutarConsulta($sql);
	}

	public function listadoAlumnosTodos(){
		$sql="SELECT a.id AS alumnoID,a.nombre,a.calle,a.num_ext,a.num_int,a.telefono,a.email,a.activo,mae.nombre AS nombreMaestro,IFNULL(a.updated_at,a.created_at) AS fechaCreacion,a.user_id,cla.nombre AS nombreClase,asist.asistenciasTotales,IFNULL(alu.asistenciasAlumnos,0) AS asistenciasAlumnos,IFNULL(pag.paginas,0) AS paginas,IFNULL(curs.paginasCurso,curso.paginasCurso) AS paginasCurso FROM alumnos a LEFT JOIN (SELECT maestro_id,nombre,id,curso_id FROM clases WHERE activo='1') cla ON a.clase_id=cla.id LEFT JOIN (SELECT curso_id,SUM(paginas) AS paginasCurso FROM cursoSecciones GROUP BY curso_id) curs ON curs.curso_id=cla.curso_id LEFT JOIN users mae ON cla.maestro_id=mae.id LEFT JOIN (SELECT rep.nombre,rep.id,SUM(dea.paginas) AS paginasCurso FROM cursos rep LEFT JOIN cursoSecciones dea ON rep.id=dea.curso_id GROUP BY rep.id) curso ON cla.curso_id=curso.id LEFT JOIN (SELECT a.clase_id,cur.nombre AS nombreCurso,MAX(a.numero_pagina) AS paginas,cl.curso_id FROM asistencias a INNER JOIN clases cl ON a.clase_id=cl.id INNER JOIN cursos cur ON cl.curso_id=cur.id WHERE cl.curso_id=a.curso_id GROUP BY a.clase_id) pag ON a.clase_id=pag.clase_id
		
		LEFT JOIN (SELECT alm.id AS alumno_id,COUNT(DISTINCT ast.id) AS asistenciasTotales FROM alumnos alm INNER JOIN asistencias ast ON alm.clase_id=ast.clase_id WHERE ast.asistencia_inicio>=DATE_FORMAT(CURDATE(), '%Y-%m-01') GROUP BY alm.id) asist ON a.id=asist.alumno_id
	...
	public function mostrarAlumno($alumnoID){
		$sql="SELECT a.id AS alumnoID,a.clase_id AS claseID,a.nombre AS nombreAlumno,a.calle,a.num_ext,a.num_int,a.telefono,a.email,a.activo,mae.nombre AS nombreMaestro,IFNULL(a.updated_at,a.created_at) AS fechaCreacion,a.user_id,cla.nombre AS nombreClase,asist.asistenciasTotales,IFNULL(alu.asistenciasAlumnos,0) AS asistenciasAlumnos,IFNULL(pag.paginas,0) AS paginas,IFNULL(curs.paginasCurso,curso.paginasCurso) AS paginasCurso,curso.nombre AS nombreCurso FROM alumnos a LEFT JOIN (SELECT maestro_id,nombre,id,curso_id FROM clases WHERE activo='1') cla ON a.clase_id=cla.id LEFT JOIN (SELECT curso_id,SUM(paginas) AS paginasCurso FROM cursoSecciones GROUP BY curso_id) curs ON curs.curso_id=cla.curso_id LEFT JOIN users mae ON cla.maestro_id=mae.id LEFT JOIN (SELECT rep.nombre,rep.id,SUM(dea.paginas) AS paginasCurso FROM cursos rep LEFT JOIN cursoSecciones dea ON rep.id=dea.curso_id GROUP BY rep.id) curso ON cla.curso_id=curso.id LEFT JOIN (SELECT a.clase_id,cur.nombre AS nombreCurso,MAX(a.numero_pagina) AS paginas,cl.curso_id FROM asistencias a INNER JOIN clases cl ON a.clase_id=cl.id INNER JOIN cursos cur ON cl.curso_id=cur.id WHERE cl.curso_id=a.curso_id GROUP BY a.clase_id) pag ON a.clase_id=pag.clase_id
