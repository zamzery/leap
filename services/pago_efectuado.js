...
function init() {
	...
	$.post( "../ajax/metodopago_efectuado.php?op=select_metodopago", function ( r ) {
...
function listar() {
	tabla = $( '#tbllistado' ).dataTable( {
		...
		"ajax": {
			url: '../ajax/pago_efectuado.php?op=listar',
			type: "get",
			dataType: "json",
			error: function ( e ) {
...
function guardaryeditar( e ) {
	...
	$.ajax( {
		url: "../ajax/pago_efectuado.php?op=guardaryeditar",
		type: "POST",
		data: formData,
		contentType: false,
		processData: false,
...
function desactivar( pagoID ) {
	bootbox.confirm( "¿Quieres desactivar el Pago?", function ( result ) {
		if ( result ) {
			$.post( "../ajax/pago_efectuado.php?op=desactivar", {pagoID: pagoID}, function ( e ) {
				bootbox.alert( e );
				tabla.clear().draw();
				tabla.ajax.reload();
			} );
...
function activar( pagoID ) {
	bootbox.confirm( "¿Quieres activar el Pago?", function ( result ) {
		if ( result ) {
			$.post( "../ajax/pago_efectuado.php?op=activar", {pagoID: pagoID}, function ( e ) {
				bootbox.alert( e );
				tabla.clear().draw();
				tabla.ajax.reload();
			} );
...
function mostrar( pagoID ) {
	$.post( "../ajax/pago_efectuado.php?op=mostrar", {pagoID: pagoID}, function ( data, statusUsuario ) {
...
function muestraHistorial() {
	...
	tablaHistorial = $( '#tblHistorial' ).DataTable( {
		...
		"ajax": {
			url: '../ajax/pago_efectuado.php?op=muestraHistorial&hist=' + pagoID,
			type: "get",
			dataType: "json",
			error: function ( e ) {
				console.log( e.responseText );
